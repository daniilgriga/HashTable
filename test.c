...
static inline uint32_t hash_CRC32_inline (const char* key)
{
    ...
    __asm__ ("crc32b %1, %0" : "+r"(hash) : "rm"(*key));
    return hash;
}
...
int main ()
{
    ...
    uint32_t result_sse = hash_CRC32_inline (test);
    uint32_t result_c = hash_CRC32 (test);

    fprintf (stderr, "SSE hash of '%s': 0x%08X\n", test, result_sse);
    fprintf (stderr, "C   hash of '%s': 0x%08X\n", test, result_c);

    if (result_sse == result_c)
